...
humaine et compatibilité avec d'éventuels scripts externes. Format identique
à l'ancien CSV historique (colonnes `Source_Artist`, `Source_Artist_ID`,
`Related_Data_Raw` au format `str(list[dict])`).
...
def main() -> None:
    ...
    records = []
    for source_artist, source_id, similar_json in rows:
        try:
            similar = json.loads(similar_json) if similar_json else []
        except (json.JSONDecodeError, TypeError):
            similar = []
        # Format historique : liste de {name, id} sans le rank
